...
namespace Z3.LinqBinding.Sudoku
{
	...
	public class SudokuByte
	{
		...
		private static readonly int[] Indices = { 0, 1, 2, 3, 4, 5, 6, 7, 8 };
		...
		public Theorem<SudokuByte> CreateTheorem(Z3Context context)
		{
			...
			for (int i = 0; i < 81; i++)
			{
				if (Cells[i] != 0)
				{
					...
					toReturn = toReturn.Where(sudoku => sudoku.Cells[idx].Equal(cellValue));
				}
			}
		...
		public static Theorem<SudokuByte> Create(Z3Context context)
		{
			...
			for (int i = 0; i < 9; i++)
			{
				for (int j = 0; j < 9; j++)
				{
					...
					var lb = (byte) 0; 
					var ub = (byte) 10;
					sudokuTheorem = sudokuTheorem.Where(sudoku => (sudoku.Cells[i1 * 9 + j1].GreaterThan(lb) && sudoku.Cells[i1 * 9 + j1].LessThan(ub)));
				}
			}
			...
			for (int b = 0; b < 9; b++)
			{
				...
				sudokuTheorem = sudokuTheorem.Where(t => Z3Methods.Distinct(indices.Select(idx => t.Cells[idx]).ToArray()));
			}
